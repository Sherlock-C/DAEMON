...
def anomaly_detection(data_type):
    ...
    with open('./detection_results_' + str(data_type[0]) +'.txt', 'w') as f:
        ...
        for file in files:
            ...
            seg_topk_name = open('./results/seg_topk_' + f_name + '.txt', 'w')
            label_name = open('./results/label_' + f_name + '.txt', 'w')
            ...
            test_data1 = DataLoader(dataset=samples_test_data, batch_size=opt.test_batchsize)

            test_data = DataLoader(dataset=samples_test_data, batch_size=1)

            model = DAEMON(opt, train_data, val_data, test_data1, test_label, device)

            train_time, epoch_time = model.train()

            model.load()

            f1, pre, rec, tp, tn, fp, fn, latency, test_time, label, _ = model.eval_result(test_data1, test_label)

            segment_score = model.calculate_outputs_and_gradients(test_data, 2000, label)


            for i in range(len(segment_score)):
                seg_topk_name.write(str(segment_score[i][0]))
                for j in range(1, len(segment_score[i])):
                    seg_topk_name.write('\t' + str(segment_score[i][j]))
                seg_topk_name.write('\n')
            seg_topk_name.close()
